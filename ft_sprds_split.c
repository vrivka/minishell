...
static int	num_of_str(char const *s)
{
	...
	while (s[i])
	{
		if (s[i] != ' ' && (s[i + 1] == ' ' || s[i + 1] == '\0') && (g_msh.check_sprds[i + 1] == ' ' || g_msh.check_sprds[i + 1] == '\0'))
			count++;
		if (s[i] == ' ' && g_msh.check_sprds[i] == '_' && (s[i + 1] == ' ' || s[i + 1] == '\0') && (g_msh.check_sprds[i + 1] == ' ' || g_msh.check_sprds[i + 1] == '\0'))
			count++;
		i++;
	}
	return (count);
